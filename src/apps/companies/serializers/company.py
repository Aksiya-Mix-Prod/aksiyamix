...
class CompanyListSerializer(CustomModelSerializer):
    ...
    class Meta:
        model = Company
        fields = ['owner', 'categories', 'owner_last_name', 'owner_first_name', 'owner_father_name',
                  'owner_phone_number1', 'owner_phone_number2', 'logo', 'video_url', 'banner', 'regions',
                  'districts', 'name', 'username', 'email', 'address', 'phone_number', 'is_verified', 'is_active',
                  'is_deleted', 'is_spammed', 'id_company', 'follower_counts', 'like_counts', 'dislike_counts',
                  'comment_counts', 'view_counts', 'spam_counts', 'branch_counts', 'product_counts', 'rating_counts',
                  'active_discount_counts', 'finished_discount_counts', 'top_tariff_counts', 'boost_tariff_counts',
                  'discount_tariff_counts', 'delivery', 'installment', 'short_description', 'long_description',
                  'web_site_url', 'longitude', 'latitude', 'balance', 'rating5', 'rating4', 'rating3', 'rating2',
                  'rating1']
...
class CompanyCreateUpdateSerializer(CustomModelSerializer):
    ...
    class Meta:
        model = Company
        exclude = ('id_company', 'follower_counts', 'like_counts', 'dislike_counts', 'comment_counts', 'view_counts',
                   'spam_counts', 'branch_counts', 'product_counts', 'branch_counts', 'rating_counts',
                   'active_discount_counts' 'finished_discount_counts' 'top_tariff_counts' 'boost_tariff_counts',
                   'discount_tariff_counts', 'balance', 'rating5', 'rating4', 'rating3', 'rating2', 'rating1')
        ...
        def update(self, instance, validate_data):
            categories = validate_data.pop('category', [])
            for attrs, value, in validate_data.items():
