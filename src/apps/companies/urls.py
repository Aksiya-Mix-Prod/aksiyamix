...
urlpatterns = [
    ...
    path('company-time-table-list/', CompanyTimeTableListViewSet.as_view({'get': 'list'}),
         name='company-time-table-list'),

    path('company-time-table-retrieve/<int:pk>/', CompanyTimeTableRetrieveViewSet.as_view({'get': 'retrieve'}),
         name='company-time-table-retrieve'),

    path('company-time-table-create/', CompanyTimeTableCreateViewSet.as_view({'post': 'create'}),
         name='company-time-table-create'),

    path('company-time-table-update/<int:pk>/', CompanyTimeTableUpdateViewSet.as_view({'patch': 'partial_update'}),
         name='company-time-table-update'),

    path('company-time-table-delete/<int:pk>/', CompanyTimeTableDeleteViewSet.as_view({'delete': 'destroy'}),
         name='company-time-table-delete'),

    # =================     URLS OF RATING COMPANY      =================
    path('company-rating/<int:pk>/', CompanyRatingViewSet.as_view({'post': 'create_rating'}),
         name='company-rating'),
    path('company-rating-update/<int:pk>/', CompanyRatingViewSet.as_view({'patch': 'update_rating'}),
         name='company-rating-update'),
    path('company-rating-retrieve', CompanyRatingViewSet.as_view({'get': 'retrieve'}),
