...
def delete_file_after_update_obj(old_instance: Model, instance):
    ...
    for field in old_instance._meta.get_fields():
        if isinstance(field, (FileField, ImageField)):
            ...
            file_field = getattr(instance, field.name, None)
            if old_file_field and file_field and old_file_field.path: 
                if default_storage.exists(old_file_field.path) and old_file_field != file_field:
